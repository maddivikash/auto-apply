...
// Anything already in Turso wins: the user may have rebuilt their profile or settings there since.
const existing = new Set(await listDocPaths(""));
let cursor: string | undefined; let n = 0;
do {
  const page = await list({ limit: 1000, cursor });
  for (const b of page.blobs) {
    if (!b.pathname.endsWith(".json")) continue;
    if (existing.has(b.pathname)) { console.log("kept newer", b.pathname); continue; }
