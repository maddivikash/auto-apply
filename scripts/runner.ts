...
async function attachResumeGeneric(page: Page, resumePath: string, notes: string[]) {
  ...
  const appeared = async () => { for (let i = 0; i < 12; i++) { const t = (await page.locator("body").innerText()).toLowerCase(); if (t.includes(fileName.toLowerCase()) || /resume[\s\S]{0,80}(success|uploaded|attached)/i.test(t)) return true; await page.waitForTimeout(500); } return false; };
