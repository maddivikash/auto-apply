...
async function main() {
  if (layoutOnly) {
    ...
    const res = await renderPdf(r, MASTER);
  ...
  const t0 = Date.now();
  const result = await tailorResume(job, MASTER);
  const resume = sanitize(result.resume);
  const warnings = [...result.warnings, ...validate(resume, MASTER)];
  ...
  const res = await renderPdf(resume, MASTER);
