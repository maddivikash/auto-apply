...
const isPublic = createRouteMatcher(["/", "/sign-in(.*)", "/sign-up(.*)", "/api/runner(.*)", "/api/internal(.*)", "/api/applications/(.*)/(pdf|screenshot)", "/api/v1(.*)", "/mcp(.*)", "/.well-known(.*)", "/openapi.json", "/connect/docs", "/privacy", "/terms"]);
