...
export default async function AnswersPage({ searchParams }: { searchParams: Promise<{ saved?: string }> }) {
  ...
  return (
    <div className="space-y-8">
      <PageHeader title="Answers" description="What the form filler already knows about you. Anything a form asks that is not covered here is sent to you as an open question." />
      <form action={saveSettingsAction} className="space-y-6">
        ...
        <div className="flex items-center justify-end gap-3">{saved && <span className="text-sm text-go">Saved.</span>}<SubmitButton pending="Saving...">Save answers</SubmitButton></div>
      </form>
    </div>
  );
}
