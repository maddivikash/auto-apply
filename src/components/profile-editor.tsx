...
export function ProfileEditor({ initial }: { initial: Profile }) {
  ...
  return (
    <form action={saveProfileAction} className="space-y-5">
      ...
      <div className="sticky bottom-4 flex justify-end"><SubmitButton pending="Saving" className="btn-primary lift">Save profile</SubmitButton></div>
    </form>
  );
}

function Section({ title, hint, children }: { title: string; hint?: string; children: React.ReactNode }) {
  return <section className="panel grid gap-6 p-5 md:grid-cols-[200px_1fr] md:p-6"><div><h2 className="text-[15px] font-semibold">{title}</h2>{hint && <p className="mt-1 text-[12.5px] leading-relaxed text-muted">{hint}</p>}</div><div className="space-y-4">{children}</div></section>;
}
